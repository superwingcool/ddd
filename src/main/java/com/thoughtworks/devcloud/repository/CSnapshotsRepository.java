...
public interface CSnapshotsRepository extends JpaRepository<CSnapshots, Long> {
    ...
    @Query(value = "SELECT s1.id FROM C_SNAPSHOTS s1 " +
            ...
            "WHERE s1.project_id in (select p.id from C_PROJECTS p where p.devcloud_project_uuid=:devcloudProjectUuid)",
            nativeQuery = true)
    List<Long> findLatestCSnapshotsIdListByGitUrl(@Param("devcloudProjectUuid") String devcloudProjectUuid);
}
