...
//class updateTopicOrUserTopicQuotaConfigHandler(private val topicOrUserTopicQuotaManagers: TopicOrUserTopicQuotaManagers) {
//
//  def updateTopicOrUserTopicQuotaConfig(sanitizedUser: Option[String], topic: Option[String], config: Properties) {
//    val producerQuota =
//      if (config.containsKey(DynamicConfig.Client.ProducerByteRateOverrideProp))
//        Some(new Quota(config.getProperty(DynamicConfig.Client.ProducerByteRateOverrideProp).toLong, true))
//      else
//        None
//    topicOrUserTopicQuotaManagers.produce.updateQuota(sanitizedUser, topic, producerQuota)
//    val consumerQuota =
//      if (config.containsKey(DynamicConfig.Client.ConsumerByteRateOverrideProp))
//        Some(new Quota(config.getProperty(DynamicConfig.Client.ConsumerByteRateOverrideProp).toLong, true))
//      else
//        None
//    topicOrUserTopicQuotaManagers.fetch.updateQuota(sanitizedUser, topic, consumerQuota)
//  }
//}
...
/**
  * TopicLimitConfigHandler将处理topic limit在zk更新.
  * 它将返回topic name和其相关的所有config
  *
  */
class TopicLimitConfigHandler(private val quotaManagers: QuotaManagers) extends QuotaConfigHandler(quotaManagers) with ConfigHandler {

  def processConfigChanges(topic: String, clientConfig: Properties) {
    updateQuotaConfig(None, Some(topic), clientConfig)
  }
}
