...
object ConfigType {
  ...
  val User = "users"
  val Broker = "brokers"
  val Topic_Limit = "topics_limit"
  val all = Seq(Topic, Client, User, Broker, Topic_Limit)
}
  ...
  def startup(): Unit = {
    ...
    configHandlers.foreach {
      case (ConfigType.User, handler) =>
        ...
        AdminUtils.fetchAllChildEntityConfigs(zkUtils, ConfigType.User, ConfigType.Topic_Limit).foreach {
          case (sanitizedUserTopic, properties) => handler.processConfigChanges(sanitizedUserTopic, properties)
        }
