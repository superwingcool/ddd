...
case class QuotaEntity(quotaId: QuotaId, sanitizedUser: String, clientIdOrTopic: String, quota: Quota)
...
class ClientQuotaManager(private val config: ClientQuotaManagerConfig,
  ...
  def recordAndMaybeThrottleForUserClientOrTopic(sanitizedUser: String, clientId: String, topic: String, value: Double, callback: Int => Unit): Int = {
    val clientSensors = getOrCreateQuotaSensors(sanitizedUser, clientId)
    val clientOrTopicSensors = isNotLimit(clientSensors.quotaEntity.quota.bound) match {
      case true => getOrCreateQuotaSensors(sanitizedUser, topic)
      case _ => clientSensors
    }
    recordAndThrottleOnQuotaViolation(clientOrTopicSensors, value, callback)
  }

  private def isNotLimit(data : Double): Boolean = {
    data <= 0 || data == Long.MaxValue
  }
  ...
  def recordAndThrottleOnQuotaViolation(clientSensors: ClientSensors, value: Double, callback: Int => Unit): Int = {
    ...
    try {
      ...
      case _: QuotaViolationException =>
        ...
        val clientMetric = metrics.metrics().get(clientRateMetricName(clientQuotaEntity.sanitizedUser, clientQuotaEntity.clientIdOrTopic))
   ...
   */
  def getOrCreateQuotaSensors(sanitizedUser: String, clientIdOrTopic: String): ClientSensors = {
    val clientOrTopicQuotaEntity = quotaEntity(sanitizedUser, clientIdOrTopic)
    // Names of the sensors to access
    ClientSensors(
      clientOrTopicQuotaEntity,
      sensorAccessor.getOrCreate(
        getQuotaSensorName(clientOrTopicQuotaEntity.quotaId),
        ClientQuotaManagerConfig.InactiveSensorExpirationTimeSeconds,
        lock, metrics,
        () => clientRateMetricName(clientOrTopicQuotaEntity.sanitizedUser, clientOrTopicQuotaEntity.clientIdOrTopic),
        () => getQuotaMetricConfig(clientOrTopicQuotaEntity.quota),
        () => measurableStat
      ),
      sensorAccessor.getOrCreate(getThrottleTimeSensorName(clientOrTopicQuotaEntity.quotaId),
        ClientQuotaManagerConfig.InactiveSensorExpirationTimeSeconds,
        lock,
        metrics,
        () => throttleMetricName(clientOrTopicQuotaEntity),
        () => null,
        () => new Avg()
      )
    )
  ...
  protected def clientRateMetricName(sanitizedUser: String, clientId: String): MetricName = {
    metrics.metricName("byte-rate", quotaType.toString,
                   "Tracking byte-rate per user/client-id or topic",
                   "user", sanitizedUser,
                   "client-id", clientId)
  }
  ...
  private def throttleMetricName(quotaEntity: QuotaEntity): MetricName = {
    metrics.metricName("throttle-time",
                       quotaType.toString,
                       "Tracking average throttle-time per user/client-id or topic",
                       "user", quotaEntity.sanitizedUser,
                       "client-id", quotaEntity.clientIdOrTopic)
  }
