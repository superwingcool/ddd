...
/**
  ...
  *     <li> <user, topic>: --entity-type users --entity-name <user-principal> --entity-type topic --entity-name <topic>
...
object ConfigCommand extends Config {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if(args.length == 0)
      CommandLineUtils.printUsageAndDie(opts.parser, "Add/Remove entity config for a topic, client, user, broker or topic_limit")
  ...
  private[admin] def parseEntity(opts: ConfigCommandOptions): ConfigEntity = {
    ...
    if (entityTypes.head == ConfigType.User || entityTypes.head == ConfigType.Client || entityTypes.head == ConfigType.Topic_Limit)
  ...
  private def parseQuotaEntity(opts: ConfigCommandOptions): ConfigEntity = {
    ...
    val reverse = types.size == 2 && (types(0) == ConfigType.Client || types(0) == ConfigType.Topic_Limit)
      ...
      if (name.isEmpty)
        ...
        entityType match {
          ...
          case ConfigType.Client =>
            ...
            name
          case ConfigType.Topic_Limit => Topic.validate(name)
            name
  ...
  class ConfigCommandOptions(args: Array[String]) {
    ...
    val entityType = parser.accepts("entity-type", "Type of entity (topics/clients/users/brokers/topics_limit)")
      .withRequiredArg
      .ofType(classOf[String])
    val entityName = parser.accepts("entity-name", "Name of entity (topic name/client id/user principal name/broker id/topic name)")
      .withRequiredArg
      .ofType(classOf[String])
    ...
    val addConfig = parser.accepts("add-config", "Key Value pairs of configs to add. Square brackets can be used to group values which contain commas: 'k1=v1,k2=[v1,v2,v2],k3=v3'. The following is a list of valid configurations: " +
      ...
      "For entity_type '" + ConfigType.Topic_Limit + "': " + DynamicConfig.TopicLimit.names.asScala.map("\t" + _).mkString(nl, nl, nl) +
      ...
      s"Entity types '${ConfigType.User}' and '${ConfigType.Topic}' may be specified together to update config for topic of a specific user.")
      .withRequiredArg
      .ofType(classOf[String])
    ...
    def checkArgs() {
      ...
      else if (entityTypeVals.size > 1 && !isUserClientOrUserTopicLimitConfigType(entityTypeVals))
        throw new IllegalArgumentException(s"Only '${ConfigType.User}' and '${ConfigType.Client}' or '${ConfigType.Topic_Limit}' entity types may be specified together")
    }

  }

  private def isUserClientOrUserTopicLimitConfigType(entityTypes: mutable.Buffer[String]) = {
    entityTypes.toSet.equals(Set(ConfigType.User, ConfigType.Client)) || entityTypes.toSet.equals(Set(ConfigType.User, ConfigType.Topic_Limit))
