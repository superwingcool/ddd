...
trait AdminUtilities {
  ...
  def changeTopicLimitConfig(zkUtils: ZkUtils, topic: String, configs: Properties)
  ...
  def changeConfigs(zkUtils: ZkUtils, entityType: String, entityName: String, configs: Properties): Unit = {
    ...
    entityType match {
      ...
      case ConfigType.Topic_Limit => changeTopicLimitConfig(zkUtils, entityName, configs)
      case _ => throw new IllegalArgumentException(s"$entityType is not a known entityType. Should be one of ${ConfigType.Topic}, ${ConfigType.Client}, ${ConfigType.Broker},  ${ConfigType.Topic_Limit}")
    }
  }
...
object AdminUtils extends Logging with AdminUtilities {
  ...
  def changeUserOrUserClientIdConfig(zkUtils: ZkUtils, sanitizedEntityName: String, configs: Properties) {
    if (sanitizedEntityName == ConfigEntityName.Default || sanitizedEntityName.contains("/clients"))
      DynamicConfig.Client.validate(configs)
    else if (sanitizedEntityName.contains("/topics_limit"))
      validateTopicLimitConfig(zkUtils, sanitizedEntityName.substring(sanitizedEntityName.lastIndexOf("/") + 1), configs)
    else
  ...
  def validateTopicLimitConfig(zkUtils: ZkUtils, topic: String, configs: Properties): Unit = {
    //Topic.validate(topic)
    if (!topicExists(zkUtils, topic))
      throw new AdminOperationException("Topic \"%s\" does not exist.".format(topic))
    DynamicConfig.TopicLimit.validate(configs)
  ...
  def changeTopicLimitConfig(zkUtils: ZkUtils, topic: String, configs: Properties) {
    Topic.validate(topic)
    validateTopicLimitConfig(zkUtils, topic, configs)
    changeEntityConfig(zkUtils, ConfigType.Topic_Limit, topic, configs)
  }
